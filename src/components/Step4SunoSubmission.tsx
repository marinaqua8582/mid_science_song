...
import { Music2, ExternalLink, Copy, Check, Send, ArrowLeft, CheckCircle2, Sparkles, FileText } from 'lucide-react';
...
export const Step4SunoSubmission: React.FC<Props> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const data: Step4Data = {
      ...
      finalSubmittedAt: new Date().toLocaleString('ko-KR')
    };
    onSubmitFinal(data);
  };
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8 space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          완성된 가사를 Suno AI에 적용하여 음원을 생성하고 (선택), 수행평가를 최종 제출합니다.
        </p>
      </div>

      {/* Suno AI Guide Box */}
      <div className="p-6 bg-gradient-to-br from-rose-50 to-orange-50 border border-rose-200/80 rounded-2xl space-y-4 shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Completion Banner */}
        {isCompleted && (
          <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-3 text-emerald-900 text-sm font-semibold">
            <CheckCircle2 className="w-6 h-6 text-emerald-600 shrink-0" />
            <span>수행평가가 성공적으로 제출되었습니다! 필요시 구글 시트 및 교사 평가 대시보드에 즉시 기록됩니다.</span>
          </div>
        )}
        ...
        <div className="pt-4 border-t border-slate-200 flex items-center justify-between">
          ...
          <button
            type="submit"
            className="px-8 py-3.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-bold rounded-xl shadow-lg transition-all flex items-center gap-2 text-base"
          >
