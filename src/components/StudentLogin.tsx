...
export const StudentLogin: React.FC<Props> = ({ roster, onLogin }) => {
  ...
  return (
    <div className="max-w-md mx-auto bg-white rounded-2xl shadow-xl border border-slate-200/80 p-6 md:p-8 transition-all">
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setSelectedClass(c);
                const numsForClass = roster.filter(r => r.classNum === c).map(r => r.studentNum);
                if (numsForClass.length > 0 && !numsForClass.includes(selectedNum)) {
                  setSelectedNum(numsForClass[0]);
                }
              }}
