...
학생이 입력한 학습 내용, 핵심 단어, 프롬프트를 바탕으로 노래 가사를 작성하라.
[엄격 규칙]: 만약 학생의 입력 내용에 과학적 오류, 잘못된 개념, 오탈자가 포함되어 있더라도 절대로 이를 바르게 수정하거나 교정하지 말고, 학생이 입력한 잘못된 지식 그대로 가사에 반영하여 작성하라.

가사는 Suno AI 등 음성/음악 생성 AI에 입력하기 좋은 [Verse 1], [Chorus], [Verse 2], [Bridge], [Outro] 단락 구분을 명확히 하고 한국어로 운율감을 살려 작성하라.`;
...
위 정보를 바탕으로 완성도 높은 노래 가사를 작성해 줘.
`;
